...
function decimoTerceiro(salarioBruto, meses) {
    const divisao = salarioBruto / 12;
    const multiplicacao = divisao * meses;
...
function calcular() {
    ...
    // CALCULOS
    const decimo = decimoTerceiro(salarioBruto, meses)
    const baseIrrf = inss(decimo);
    ...
    proventosTotaisTabela.innerHTML = `${decimo.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`;
    proventosSalarioBrutoTabela.innerHTML = `${decimo.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`;
